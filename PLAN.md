...
独立 C-fracture schema、36-case 冻结协议和本地 P1 AT2 调试内核已经实现；
这三者目前不是可运行的 Phase-1 数据生成器。独立审计将状态判为 `NO-GO`：

1. v3 加载适配器已经能在任意 `s` 上唯一计算 P1-P4 的远场应力张量和逐壁面
   facet 卸荷，并修正了旧配置中会把右墙误作冠部的坐标语义；development-only
   scheduled API 已接入本地内核，P1 旧路径、P2/P3 变远场和 P4 分区卸荷均有反例；
   development-only 轨迹适配器也已补上自适应减半、完整接受/失败账本和 schema v3
   保存复验，但尚未通过协议规模与三网格门，不能发布为训练标签；
2. 裂纹带 Distance/Threshold 网格和实际 `h/ell` 硬审计已经补齐；SENT/SENS
   也已生成六档真实 Gmsh 显式双裂面网格，严格对齐 `h/ell=1/2,1/4,1/8`、
   七类边界 facet 与独立 0D 裂尖身份。成对 coarse fixed-`d=0` 三状态实跑已经
   6/6 通过适用平衡门，峰值 RSS 约 402 MiB；SENT/SENS 的完整 fixed-`d=0`
   轨迹耗时下界分别约 54.24 h/58.79 h，因此仍须先做可重启耦合短前缀和性能优化；
3. 求解器已从最终同一 `(u,d)` 状态输出结点内力、壁面力、约束 DOF 和规定位移；
   schema v3 会从原始量重算反力、自由 DOF 平衡、壁面/远场梯形功、累计功和能量
   不平衡，并绑定几何/拓扑/DOF/逐步载荷身份。development-only 轨迹适配已逐接受步
   重放冻结 schedule 并核对 stress/facet order/release，也已接入全局力矩、自适应减半、
   失败账本和保存-重载-复验；它仍因粗网格和外部基准门未完成而禁止作为正式标签；
4. MOOSE 官方 `crack2d_iso` 自测已在固定且已推送的 MOOSE/项目提交上执行通过并
   留存哈希证据；本地与 MOOSE 在同一 TRI3 网格的 3 个完好弹性与 3 个固定
   非均匀 P1 损伤平衡状态上亦通过 `1e-6` 门；这两项都不能替代耦合裂纹演化验证；
5. 最终 `(u,d,H)` 后损伤状态一致性反例已修复；通用位移控制 BVP、SENT/SENS
   `[y,z]`/node-major 边界适配、现场拓扑哈希重算和最多 12 状态的非授权完整体
   探针已经闭合，且同一 pushed HEAD 下的 SENT+SENS 成对真实证据已落盘。协议状态
   仍为 `ABSTAIN_NOT_RUN`：该证据固定 `d=0`，不满足每 case 10 步正式 timing probe，
   也不授权耦合轨迹。在耦合短前缀、性能优化、三网格和资源门通过前，仍禁止生成
   36 条标签或启动模型训练。

因此当前允许范围仍是 kernel/schema/benchmark 调试和非标签 toy case；
`solver_implemented=false` 表示“完整协议求解器未实现”，不是否认已经存在的
内部数值内核。
