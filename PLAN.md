...
1. v3 加载适配器已经能在任意 `s` 上唯一计算 P1-P4 的远场应力张量和逐壁面
   facet 卸荷，并修正了旧配置中会把右墙误作冠部的坐标语义；但它尚未接入
   位移/损伤求解器，因此当前内核实际仍只能运行固定远场应力与全洞壁同步卸荷；
2. 裂纹带 Distance/Threshold 网格和实际 `h/ell` 硬审计已经补齐，但尚未做
   协议尺寸的单案例资源试算；
3. 约束反力、壁面/远场梯形路径功、拒绝尝试回滚和增量能量诊断的纯函数原语
   已实现并测试；求解器仍未输出与最终 `(u,d)` 一致的结点力，也未把这些量接入
   累计功、全局力矩平衡、trajectory schema、自适应减半和失败账本；
